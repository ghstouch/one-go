...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		Database: DatabaseConfig{
			...
			SQLitePath:   getEnv("SQLITE_PATH", "./storage/omniroute.db"),
			PostgresHost: getEnv("POSTGRES_HOST", "localhost"),
			PostgresPort: getEnv("POSTGRES_PORT", "5432"),
			PostgresUser: getEnv("POSTGRES_USER", "omniroute"),
			PostgresPass: getEnv("POSTGRES_PASSWORD", "omniroute"),
			PostgresDB:   getEnv("POSTGRES_DB", "omniroute"),
			PostgresSSL:  getEnv("POSTGRES_SSLMODE", "disable"),
		},
		JWT: JWTConfig{
			Secret:      getEnv("JWT_SECRET", "omniroute-secret-key"),
			ExpiryHours: getEnvInt("JWT_EXPIRY_HOURS", 24),
			Issuer:      getEnv("JWT_ISSUER", "omniroute-go"),
		},
		Admin: AdminConfig{
